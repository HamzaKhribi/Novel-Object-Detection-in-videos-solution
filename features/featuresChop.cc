...
mxArray *process(const mxArray *mximage,const mxArray *mxorientation,const mxArray *mxsbin) {
  double *im = (double *)mxGetPr(mximage);
  double *orient=(double *)mxGetPr(mxorientation);
 ...
 /* if (mxGetNumberOfDimensions(mximage) != 3 ||
  ...
  for (int x = 1; x < visible[1]-1; x++) {
	...
	double *ori=orient + min(x, dims[1]-2)*dims[0] + min(y, dims[0]-2);
	int best_o=0;
 if (*ori>0 && *ori<=20)
                 best_o=0;

       	else if (*ori>20 && *ori<=40)
		best_o=1;
	
	else if (*ori>40 && *ori<=60)
		best_o=2;

	else if (*ori>60 && *ori<=80)
		best_o=3;
		
	else if (*ori>80 && *ori<=100)
		best_o=4;

	else if (*ori>100 && *ori<=120)
		best_o=5;

	else if (*ori>120 && *ori<=140)
		best_o=6;

	else if (*ori>140 && *ori<=160)
		best_o=7;

	else if (*ori>160 && *ori<=180)
		best_o=8;

	else if (*ori<=0 && *ori>-20)
                 best_o=9;
  ...
  for (int x = 0; x < out[1]; x++) {
    for (int y = 0; y < out[0]; y++) {
      ...
      // contrast-insensitive features[M_red m_red or_red ft_red PC_red EO_red]
      src = hist + (x+1)*blocks[0] + (y+1);
      for (int o = 0; o < 9; o++) {
